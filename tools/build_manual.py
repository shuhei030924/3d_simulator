...
def gui_section() -> str:
  ...
  各ダイアログの上部には、その工程が何をするかの<strong>1 行ヒント</strong>が
  表示されます。代表的な工程の設定画面を示します。</p>
  <div>
...
MANUAL_CSS = """
 ...
 nav .cat-toggle { cursor:pointer; user-select:none; display:flex; align-items:center; gap:5px; }
 nav .cat-toggle::before { content:"\\25be"; font-size:10px; color:var(--muted); width:10px; }
 nav .cat-toggle.collapsed::before { content:"\\25b8"; }
 nav .cat-toggle:hover { color:var(--accent); }
...
(function(){
  ...
  if(tb) tb.addEventListener('click',function(){
    ...
    updTheme();});
  updTheme();
  // 文字サイズ調整（本文を 80〜160% でズーム, localStorage 永続化）
  var FS_KEY='semisim-fontscale';
  var mainEl=document.querySelector('main');
  var fs=parseInt(localStorage.getItem(FS_KEY)||'100',10); if(isNaN(fs)) fs=100;
  function applyFs(){ fs=Math.max(80,Math.min(160,fs));
    if(mainEl) mainEl.style.zoom=(fs/100);
    localStorage.setItem(FS_KEY,String(fs)); }
  applyFs();
  var fdb=document.getElementById('fontDecBtn');
  ...
  var catGroups=[];
  Array.prototype.slice.call(document.querySelectorAll('nav .cat')).forEach(function(cat){
    cat.classList.add('cat-toggle'); cat.setAttribute('role','button'); cat.tabIndex=0;
    var group=[]; var el=cat.nextElementSibling;
    while(el && !el.classList.contains('cat')){ group.push(el); el=el.nextElementSibling; }
    function toggle(){ var col=cat.classList.toggle('collapsed');
      group.forEach(function(g){ g.style.display=col?'none':''; }); }
    cat.addEventListener('click',toggle);
    cat.addEventListener('keydown',function(e){
      if(e.key==='Enter'||e.key===' '){ e.preventDefault(); toggle(); } });
    catGroups.push({cat:cat,group:group});
  });
  function expandAllCats(){ catGroups.forEach(function(c){
