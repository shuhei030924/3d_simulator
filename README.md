...
- **プリセットレシピ**: 代表的な 18 フロー（ダマシン・MOSFET・LDD MOSFET・KOH・DRIE・TSV 貫通ビア・サリサイドゲート・薄化 3D-IC・STI 素子分離・FinFET フィン形成（high-k/メタルゲート）・W コンタクトプラグ・欠陥再現＝マイクロマスキング/ボイド開口 等）をメニューから即読込（`semisim/presets.py`）。
