...
class PVD(Process):
    ...
    def apply(self, wafer: Wafer) -> None:
        ...
        if self.growth_fraction < 1.0:
            # 肩カスプ・封止偽像の除去は成膜完了時にのみ適用する。途中
            # フレームにも適用する案は検証済みで不採用: カスプは「最終形を
            # 連続膜にする補正」であり、異なる成長比率のカスプ同士は入れ子に
            # ならないため、フレーム間で材料が消える非単調（最大 424vox）が
            # 機械検証で観測された。プレフィックスのみなら単調性は自明に成立。
            return
